...
---@param opts? table Optional dependency overrides
---@return { check_require_approval: fun(cmd: string|nil): boolean }
function M.new(pauselist_rules, opts)
  opts = opts or {}
  local get_string_parser = opts.get_string_parser or vim.treesitter.get_string_parser
  local rules = pauselist_rules or {}

  local parse_command_entries, resolve_proxies, extract_commands

  ---Parse bash source into command entries using treesitter
  ---@param source string
  ---@return { name: string, args: string[] }[]|nil
  parse_command_entries = function(source)
    local ok_p, parser = pcall(get_string_parser, source, "bash")
    if not ok_p then
      return nil
    end
  ...
  resolve_proxies = function(entries, depth)
